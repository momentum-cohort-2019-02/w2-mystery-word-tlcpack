...
# breaking out above function for testing
def is_letter(new_guess):
    all_letters = "abcdefghijklmnopqrstuvwxyz"
    all_letters += all_letters.upper()
    new_guess = input("Enter your guess (one letter): ")
    new_guess = new_guess.casefold()
    if new_guess.lower() in all_letters:
        return True

def correct_length(new_guess):
    if len(new_guess) == 1:
        return True

def in_current_guesses(new_guess):
    current_guesses = ['a', 'b', 'c', 'e', 'f']
    if new_guess.lower() not in current_guesses:
        return True
